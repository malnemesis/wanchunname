...
INNER JOIN estados ON cpu.EST_CODIGO = estados.EST_CODIGO WHERE C_TIPO NOT IN('NINGUNO',' ')";       
             ...
             ?>
                <tr>
                  <td><a class="btn btn-info" data-toggle="modal" data-target="#modalEdicion" onclick="agregaformcpu('<?php echo $datos?>')" role="button"><i class="fa fa-fw fa-pencil" aria-hidden="true" title="Editar"></i></a></td>
                  <td><a class="btn btn-danger" role="button"><i class="fa fa-trash" aria-hidden="true" title="Eliminar" onclick="preguntarSiNoCPU('<?php echo $cpu[0] ?>','<?php echo $cpu[1] ?>')"></i></a></td>
