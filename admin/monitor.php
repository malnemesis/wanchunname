...
<!-- Modal para edición monitor-->
   ...
   <div class="modal fade" id="modalEdicion">
      ...
      <input type="text" hidden="" id="MON_CODIGO" >
    ...
    <label>Estado</label>
          ...
          <div class="form-group col-md-6">
            <label>Marca</label>
            <input type="text" class="form-control" id="MON_MARCAU"/>
          </div>
          </div>
          <div class="row">
          <div class="form-group col-md-6">
            <label>Modelo</label>
            <input type="text" class="form-control" id="MON_MODELOU"/>
          </div>
          <div class="form-group col-md-6">
            <label>Serie</label>
            <input type="text" class="form-control" id="MON_SERIEU"/>
          </div>
          <div class="form-group col-md-6">
            <label>Código de Activo Fijo</label>
            <input type="text" class="form-control" id="MON_CODACTFIJU"/>
          </div>
          </div>
          <div class="form-group">
            <label>Observación</label>
            <input type="text" class="form-control" id="MON_OBSERVACIONU"/>
          </div>
      ...
      <div class="modal-footer">
        <button class="btn" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i> <strong>Cerrar</strong></button>
        <button type="button" class="btn btn-primary" data-dismiss="modal" id="actualizamonitor">Actualizar Monitor</button>
      </div>
    </div>
  </div>
</div>
...
$('#actualizamonitor').click(function(){
  actualizamonitor();
});
...
$(document).ready(function(){
      var consulta;
      $("#MON_CODACTFIJ").focus();
      $("#MON_CODACTFIJ").keyup(function(e){
             consulta = $("#MON_CODACTFIJ").val();
                        $.ajax({
                              type: "POST",
                              url: "../admin/action/vcodaf.php",
                              data: "MON_CODACTFIJ="+consulta,
                              dataType: "html",
                              error: function(){
                                    alert("error petición ajax");
                              },
